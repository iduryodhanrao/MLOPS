...
class DBAgent:
    def __init__(self, config_file: str, db_file: str):
        ...
        self.task_results = {}  # Dictionary to store results of previous tasks
        self.history = []  # List to store all commands and their results
        ...
        async for msg in stream.receive():
            ...
            if text := msg.text:
                if len(all_responses) < 2 or not all_responses[-2].text:
                    print('### Text')
                print(text, end='')
                self.history.append({"command": "text", "response": text})  # Save to history
                ...
                for fc in tool_call.function_calls:
                    ...
                    if callable(tool_impl):
                        try:
                            ...
                            self.task_results[fc.name] = result
                            self.history.append({"command": fc.name, "response": result})  # Save to history
                ...
                for part in msg.server_content.model_turn.parts:
                    if code := part.executable_code:
                        print(f'### Code\n```\n{code.code}\n```')
                        self.history.append({"command": "code", "response": code.code})  # Save to history
                    elif result := part.code_execution_result:
                        print(f'### Result: {result.outcome}\n'
                              f'```\n{pformat(result.output)}\n```')
                        self.history.append({"command": "execution_result", "response": result.output})  # Save to history
    ...
    async def run_session(self, message: str):
        ...
        self.history.append({"command": "user_input", "message": message})  # Save user input to history
